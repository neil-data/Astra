...
from models import SpaceWeatherForecast, ProcessedSpaceWeatherFeature, RawSpaceWeatherObservation
...
async def create_forecast(
    ...
    if feature:
        ...
        proton_flux       = feature_vector.get("proton_flux_10mev")
    else:
        ...
        if not raw:
            raise HTTPException(
                status_code=404,
                detail="No data available. Push observations via POST /api/v1/history first."
            )

        kp_index          = raw.kp_index or 2.0
        storm_probability = 0.2
        confidence        = 0.6
        feature_id        = None
        proton_flux       = raw.proton_flux_10mev
    ...
    forecast = SpaceWeatherForecast(
        ...
        predicted_proton_flux             = proton_flux,
